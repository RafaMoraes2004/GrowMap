...
@Component({
  ...
  imports: [
    CommonModule,     
    RouterModule,
    FormsModule, // FormsModule é necessário
  ],
  providers: [ DatePipe ] // DatePipe é necessário
})
export class UsuariosComponent implements OnInit {
  ...
  newUser: {
    ...
    entryDate: Date;
    password?: string;
  } = { name: '', role: '', email: '', entryDate: new Date() };
  ...
  // --- Lógica do Modal "Add User" (MODIFICADA) ---
  openAddUserModal(): void { 
    this.newUser = { name: '', role: '', email: '', entryDate: new Date(), password: '' }; 
  ...
  /**
   * 👇👇👇 MÉTODO 'addUser' ATUALIZADO 👇👇👇
   * Agora ele recebe o formulário (NgForm) como argumento.
   */
  addUser(form: NgForm): void { 
    // Verifica se o formulário é inválido
    if (form.invalid) {
      // Se for inválido, marca todos os campos como "tocados" (touched)
      // Isso vai fazer com que as mensagens de erro apareçam no HTML
      Object.keys(form.controls).forEach(key => {
        form.controls[key].markAsTouched();
      });
      return; // Não continua se o formulário estiver inválido
    }

    // Se o formulário for VÁLIDO:
    const newUserToAdd: IUser = { 
    ...
    console.log('Usuário adicionado:', newUserToAdd); 
  }
  // --- Fim da Lógica "Add User" ---

  // --- Lógica dos outros Modais/Painéis ---
  ...
  suggestRoadmap(roadmap: IAvailableRoadmap): void { if (this.selectedUser) { const alreadySuggested = this.selectedUser.suggestedRoadmaps?.some(s => s.roadmapId === roadmap.id); if (!alreadySuggested) { const newSuggestion: IRoadmapSuggestion = { roadmapId: roadmap.id, roadmapName: roadmap.name, status: 'pending', suggestedDate: new Date() }; if (!this.selectedUser.suggestedRoadmaps) { this.selectedUser.suggestedRoadmaps = []; } this.selectedUser.suggestedRoadmaps.push(newSuggestion); console.log(`Roadmap "${roadmap.name}" sugerido para ${this.selectedUser.name}`); } else { console.log(`Roadab "${roadmap.name}" já foi sugerido.`); } this.closeSuggestRoadmapPanel(); } }
}